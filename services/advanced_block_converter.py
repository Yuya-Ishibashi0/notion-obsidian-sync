...
class AdvancedBlockConverter:
        ...
        for block in blocks:
            try:
                ...
                if converted_block:
                    ...
                    # 成功した変換を追跡
                    self.limitation_tracker.track_block_conversion(block.type, success=True)
            except Exception as e:
                self.logger.warning(f"ブロック変換エラー ({block.type}): {str(e)}")
                # エラーを追跡
                self.limitation_tracker.track_block_conversion(
                    block.type, success=False, 
                    warning_message=f"変換エラー: {str(e)}"
                )
    ...
    def get_conversion_report(self) -> Dict[str, Any]:
        ...
        Returns:
            変換品質レポート
        """
        return self.limitation_tracker.generate_conversion_quality_report()
    
    def get_user_warnings(self) -> List[str]:
        """
        ユーザー向け警告を取得
        ...
        Returns:
            ユーザー向け警告メッセージのリスト
        """
        return self.warning_system.generate_user_friendly_warnings()
    ...
    def _convert_callout_block(self, block: NotionBlock) -> str:
        """コールアウトブロックの変換"""
        # 制限を追跡
        self.warning_system.warn_limited_block(
            "callout", "背景色とカスタムスタイルは失われます", f"ブロックID: {block.id}"
        )
    ...
    def _convert_toggle_block(self, block: NotionBlock) -> str:
        ...
        # 制限を追跡
        self.warning_system.warn_limited_block(
            "toggle", "Notion固有のスタイルは失われます", f"ブロックID: {block.id}"
        )
    ...
    def _convert_database_block(self, block: NotionBlock) -> str:
        ...
        # 制限を追跡
        self.warning_system.warn_limited_block(
            "child_database", "データベース機能、フィルタリング、ソート、ビュー切り替えは失われます", 
            f"ブロックID: {block.id}"
        )
    ...
    def _convert_column_list_block(self, block: NotionBlock) -> str:
        ...
        # 制限を追跡
        self.warning_system.warn_limited_block(
            "column_list", "カラム幅調整、レスポンシブレイアウトは失われます", 
            f"ブロックID: {block.id}"
        )
    ...
    def _convert_synced_block(self, block: NotionBlock) -> str:
        ...
        # 制限を追跡
        self.warning_system.warn_limited_block(
            "synced_block", "リアルタイム同期、双方向更新は失われます", 
            f"ブロックID: {block.id}"
        )
    ...
    def _handle_unsupported_block(self, block: NotionBlock) -> Optional[str]:
        """サポートされていないブロックの処理"""
        # 警告システムに通知
        self.warning_system.warn_unsupported_block(block.type, f"ブロックID: {block.id}")
